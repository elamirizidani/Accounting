...
function Proforma() {
    ...
    const {quotation,loadQuotation,lpos,loadLpos} = useAuthStore()
   

    const deleteQuotation = async (id) => {
        try {
            const res = await deleteData('lpo', id);
            if (res.status === 200) {
    ...
    const exportQuotation = (lpo) => {
        if (!lpo || !lpo?.items) {
            alert('LPO data is incomplete');
            return;
        }

        const printWindow = window.open('', '_blank');
        const subtotal = lpo?.items.reduce((sum, item) => sum + (item.total || 0), 0);
        const taxAmount = lpo?.enableTax ? subtotal * 0.18 : 0;
        const total = subtotal + taxAmount;

        printWindow.document.write(`
            <html>
                <head>
                    <title>lpo ${lpo?._id || 'N/A'}</title>
                    <style>
                ...
                <body>
                    <div class="header">
                        <h1>lpo</h1>
                        <p>lpo #: ${lpo?.lpoId || 'N/A'}</p>
                        <p>Date: ${lpo?.lpoDate ? new Date(lpo?.lpoDate).toLocaleDateString() : 'N/A'}</p>
                    </div>
                    
                    <div class="details">
                        <div class="bill-section">
                            <h3>From:</h3>
                            <p><strong>${lpo?.billedBy?.name || 'N/A'}</strong></p>
                            <p>${lpo?.billedBy?.address || ''}</p>
                            <p>Phone: ${lpo?.billedBy?.phone || ''}</p>
                            <p>Email: ${lpo?.billedBy?.email || ''}</p>
                        </div>
                        <div class="bill-section right">
                            <h3>To:</h3>
                            <p><strong>${lpo?.billedTo?.name || 'N/A'}</strong></p>
                            <p>${lpo?.billedTo?.address || ''}</p>
                            <p>Phone: ${lpo?.billedTo?.phone || ''}</p>
                            <p>Email: ${lpo?.billedTo?.email || ''}</p>
                        </div>
                    </div>
                        ...
                        <tbody>
                            ${lpo?.items?.map(item => `
                                <tr>
                                    ...
                                    <td>${(item.unitCost || 0).toFixed(2)} ${lpo?.currency || 'USD'}</td>
                                    <td>${item.vat || 0}%</td>
                                    <td>${(item.total || 0).toFixed(2)} ${lpo?.currency || 'USD'}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
                    ...
                    <div class="total-section">
                        <p>Subtotal: ${subtotal.toFixed(2)} ${lpo?.currency || 'USD'}</p>
                        ${lpo?.enableTax ? `
                            <p>Tax (18%): ${taxAmount.toFixed(2)} ${lpo?.currency || 'USD'}</p>
                        ` : ''}
                        <p><strong>Total: ${total.toFixed(2)} ${lpo?.currency || 'USD'}</strong></p>
                    </div>
            ...
            <div className="offcanvas offcanvas-end" tabIndex="-1" id="filterOffcanvas" aria-labelledby="filterOffcanvasLabel">
                ...
                <div className="offcanvas-body">
                    <form id="proformaFilterForm">
                        ...
                        <div className="mb-4">
                            <label className="form-label">LPO Id</label>
                                ...
                                {lpos?.map((proforma, i) => {
                                    ...
                                    return (
                                        <tr key={proforma._id || i}>
                                            ...
                                            <td>{moment(proforma?.lpoDate).format('MMM D, YYYY')}</td>
                                            <td>
