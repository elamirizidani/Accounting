...
const ViewLPO = ({ show, handleClose, quotation }) => {
  ...
  const handlePrintToPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>LPO ${quotation?.lpoId || ''}</title>
          <style>
            ${styleSheets}
            @media print {
          ...
          <div ref={componentRef} className="print-section p-4" style={{backgroundColor:'transparent'}}>
            ...
            <div className="d-flex justify-content-between mb-4">
              ...
              <div className="text-start  mt-3">
                <h5 className="fw-bold mb-1">{quotation?.lpoId}</h5>
                <p className="fw-bold mb-1"><strong>Date:</strong> {formatDate(quotation?.lpoDate || quotation?.createdAt)}</p>
                {/* <p className="mb-1"><strong>Status:</strong> <span className="text-capitalize">{quotation?.status}</span></p> */}
              </div>
            </div>
...
{/* 
      ...
      <Modal.Footer className="no-print">
        ...
        <PDFDownloadButton
          ...
          filename={`LPO_${quotation?.lpoId || 'Unknown'}_${quotation?.billedTo?.name?.replace(/[^a-zA-Z0-9]/g, '_') || 'Customer'}.pdf`}
